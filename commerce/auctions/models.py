...
class Offers(models.Model):
    offerer = models.ForeignKey(User, related_name="offered_by", verbose_name="Offered By", on_delete=models.CASCADE)
    offer_amount = models.PositiveIntegerField(blank=False, verbose_name="Offer", default=1, validators=[MinValueValidator(1)])
...
class Listing(models.Model):
    ...
    description = models.CharField(max_length=1000, blank=False, verbose_name="Description")
    price = models.ForeignKey(Offers, verbose_name="Price", related_name="price", on_delete=models.CASCADE)
    ...
    categories = models.CharField(blank=True, choices=CATEGORY_CHOICES, max_length=2, default=CATEGORY_CHOICES[9][1], verbose_name="Category" )
...
class Comments(models.Model):
    user = models.ForeignKey(User, related_name="commented_by", verbose_name="Commented By", on_delete=models.CASCADE)
    product = models.ForeignKey(Listing, related_name="commented_on", verbose_name="Commented On", on_delete=models.CASCADE)
    comment = models.CharField(max_length=500, blank=False, verbose_name="Comment" )
    commented_at = models.DateField(default=datetime.now(), verbose_name="Comment created at")
    
    def __str__(self):
         return f'Comment by {self.user.username} on "{self.product.title}": {self.comment}'
